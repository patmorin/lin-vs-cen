...
\begin{compactenum}
  ...
  \begin{compactenum}[(Q1)]
    \item $p$ is adjacent to $\{(x-1,y), (x,y-1)\}$ and $q$ is adjacent to $(x+1,y),(x,y+1)$;
    \item $p$ is adjacent to $\{(x,y+1), (x-1,y)\}$ and $q$ is adjacent to $(x,y-1),(x+1,y)$; or
    \item $p$ is adjacent to $\{(x-1,y),(x+1,y)\}$ and $q$ is adjacent to $\{(x,y-1),(x,y+1)\}$.\label{q_iii}
  \end{compactenum}
\end{compactenum}

Let $G$ be an $a\times b$ pseudogrid.  For an edge $vw$ of $G_{a\times b}$, we let $P_{vw}:=\overline{P}_{vw}-\{v,w\}$ denote the (possibly empty) subpath containing the internal vertices of $\overline{P}_{vw}$.  For each vertex $v$ of $G_{a\times b}$ of degree less than $4$ we define $P_{v}$ to be the $1$-vertex path that contains only $v$.  In this way, $\mathcal{P}:=\{V(P_v):v\in V(G_{a\times b})\}\cup\{V(P_{vw}):vw\in E(G_{a\times b})\}$ is a partition of $V(G)$ into induced paths.  We call $\mathcal{P}$ the \defin{grid-partition} of $G$ and we denote it by $(P_\mu:\mu\in V(G_{a\times b})\cup E(G_{a\times b}))$.  This defines a mapping $\delta:V(G)\to V(G_{a\times b})\cup E(G_{a\times b})$ where $u\in V(P_{\delta(u)})$ for each $u\in v(G)$.
...
As part of our proof, we use the operation of \defin{deleting} a row (or column) $R$ of $G$, which works as follows:  We remove all edges of $R$ from $G$ and eliminate any isolated vertices.  If this produces vertices of degree $1$ (which happens when $R$ is the first or last row of $G$) then we repeatedly remove degree-$1$ vertices until none remain.  If $G$ is an $a\times b$ pseudogrid and we delete some row $R$, then the resulting graph is an $a\times (b-1)$ pseudogrid.  Similarly, if we delete column $C$ of $G$, then the resulting graph is an $(a-1)\times b$ pseudogrid.


\subsection{Pseudogrids with Exclusively Frequent Colours}

Let $G$ be a $k\times k$ pseudogrid with grid partition $\mathcal{P}:=(P_\mu:\mu\in\VE(G_{k\times k}))$ and let $\varphi:V(G)\to\{1,\ldots,c\}$ be a vertex colouring of $G$.  For any $\mu\in\VE(G_{k\times k})$, we let $\varphi_{\mathcal{P}}(\mu):=\{\phi(v):v\in V(P_\mu)\}$.  For any colour $\alpha\in\{1,\ldots,c\}$, define $\varphi^{-1}(\alpha):=\{v\in V(G):\varphi(v)=\alpha\}$ and define $\varphi_\mathcal{P}^{-2}(\alpha):=\{\mu\in \VE(G_{k\times k}):\alpha\in\varphi_\mathcal{P}(\mu)$.  For any colour set $A\subseteq\{1,\ldots,c\}$ define $\varphi^{-1}(A):=\bigcup_{\alpha\in A}\varphi^{-1}(\alpha)$ and $\varphi_\mathcal{P}^{-2}(A):=\bigcup_{\alpha\in A}\varphi_{\mathcal{P}}^{-2}(\alpha)$.
...
\begin{lem}
  Let $0 < \epsilon < 1$, let $k\ge 1/\epsilon$ be an integer, let $G$ be a $k\times k$ pseudogrid and let $\varphi:V(G)\to\{1,\ldots,\lfloor\epsilon k\floor\}$ be a vertex colouring of $G$.
  Then $G$ contains a $k'\times k'$ pseudogrid $G'$ with $k'\ge k/2$ that has a grid-partition $\mathcal{P}':=(P'_\mu:\mu\in \VE(G_{k'\times k'})$ such that
  for any $A\subseteq \varphi(V(G'))$, $|\varphi_{\mathcal{P}'}^{-2}(A)| \ge |A|/2\epsilon$.
\end{lem}

\begin{proof}
  The proof is by induction on $|\varphi(V(G))|$, the number of colours used by the colouring $\varphi$.  If there exists no $A\subseteq\{1,\ldots,\lfloor\epsilon k\rfloor\}$ with $\varphi_{\mathcal{P}}^{-2}(A) < |A|/2\epsilon$ then taking $G':=G$ and $\mathcal{P}':=\mathcal{P}$ satisfies the requirements of the lemma. Now assume that such a set $A$ exists.

  We will remove a set $R$ of rows and a set $C$ of columns from $G$ with $|R|=|C|\le |A|/2\epsilon$.  For each $v:=(i,j)\in V(G_{k\times k})$ with $A\cap\varphi(v)\neq\emptyset$, we include row $j$ in $R$ and column $i$ in $C$.  For each edge $vw\in E(G_{k\times k})$ with $A\cap\phi()\alpha\in\Phi(vw)$, we include the row of $G$ that contains $P_{vw}$ in $R$ or we include the column of $G$ that contains $P_{vw}$ in $C$.  Finally, we add arbitrary rows to $R$ or columns to $C$ to ensure that $|R|=|C|$.  Now we remove all rows in $R$ and all columns in $C$ from $G$ to obtain a $k_0\times k_0$ pseudogrid $G_0$ with $k_0\ge k-|A|/2\epsilon$ and such that $\varphi(V(G_0))\cap A=\emptyset$.  In particular, $|\varphi(V(G_0))|\le |\varphi(V(G))|-|A|$.

  \todo[inline]{Now apply induction......}
\end{proof}

\begin{lem}
  Let $C>1$ be an integer, let $G$ be a $k\times k$ pseudogrid with grid-partition $\mathcal{P}$, and let $\varphi:V(G)\to\{1,\ldots,c\}$ be a vertex colouring of $G$ such that, for any $A\subseteq\varphi(G)$, $|\varphi_{\mathcal{P}'}^{-2}(A)| \ge C\cdot|A|+1$. Then there exists a partial colouring $\phi:\VE(G_{k\times k})\to\{1,\ldots,c,\perp\}$ with the following properties:
  \begin{compactenum}[(i)]
    \item For each $\mu\in\VE(G_{k\times k})$, $\phi(\mu)=\perp$ or $\phi(\mu)\in\varphi(\mu)$.
    \item For each $\alpha\in\varphi(V(G))$, $|\{\mu\in\VE(G_{k\times k}):\phi(\mu)=\alpha\}|\ge C$.
  \end{compactenum}
\end{lem}

\begin{proof}
  Use Hall's Theorem to find $C$ matchings such that each colour $\alpha$ appears in all $C$ matchings and each element of $\VE(G_{k\times k})$ appears in at most one matching.
\end{proof}

\subsection{Finding Paths Through Well-Separated Vertex/Edge Pairs}
...
The \defin{$r$-box} centered at an edge $vw$ of $G_{a\times b}$ is $B_r(vw):=B_r(v)\cup B_r(w)$.  Observe that, for any $\mu\in V(G_{a\times b})\cup E(G_{a\times b})$,  $v\in B_r(u)$ if and only if $u\in B_r(v)$.  Any $r$-box $B_r(\mu)$ defines an induced subgraph that we denote by $G_r(\mu):=G_{a\times b}[B_r(\mu)]$.  We extend these definitions to vertices of an $a\times b$ pseudogrid $G$ so that
\[
   \tilde{B}_r(v) := \bigcup_{\mu\in V(G_r(\delta(v)) \cup E(G_r(\delta(v))} V(P_\mu)
\]
and $\tilde{G}_r(v):=G[\tilde{B}_r(v)]$.
...
\begin{lem}
  Let $G$ be a $k\times k$ pseudogrid and let $\varphi:V(G)\to\{1,\ldots,c\}$ be a colouring of $G$ such that, for each $\alpha\in\{1,\ldots,C\}$, the set $S_\alpha:=\{\mu\in V(G_{k\times k})\cup E(G_{k\times k}): \alpha\in\Phi(\mu)\}$ has size at least $C$.  Then there exists $S\subseteq V(G_{k\times k})\cup E(G_{k\times k})$ such that
  \begin{compactenum}[(i)]
    \item \label{hits_both} $|S\cap S_\alpha|\ge 2$ for each $\alpha\in\{1,\ldots,c\}$; and
    \item \label{spread_out} $|B_r(\mu)\cap S|\le 2$ for each $\mu\in S$.
  \end{compactenum}
\end{lem}

\begin{proof}
  We construct $S$ using a two round process.  We start with an initially empty set $S_1$ and repeat the following for each $\alpha= 1,\ldots,c$:
  If there exists distinct $\mu_1,\mu_2\in S_{\alpha}$ such that
  \begin{compactenum}[(a)]
    \item $\alpha\in\Phi(\mu_1)\cap\Phi(\mu_2)$;
    \item $\mu_1\not\in B_r(\mu_2)$; and
    \item $\{\mu_1,\mu_2\}\cap \bigcup_{\mu\in S} B_{r}(\mu)=\emptyset$
  \end{compactenum}
  then include $\mu_1$ and $\mu_2$ in $S_1$ and declare this a \defin{success} for colour $\alpha$.  Otherwise, declare this a failure for colour $\alpha$.

  At the end of this process, the resulting set satisfies (\ref{spread_out}) but is only guaranteed to satisfy (\ref{hits_both}) for the successful colours $\alpha$.  We now use the second round to create a set $S_2$ to fix this. Our strategy is to use \cref{hall_vees} to find two matchings between the failed colours and the boxes in $\{B_r(\mu):\mu\in S_1\}$ that contain those colours.  Each missing colour is matched in both matching, but each box is used in at most one of the matchings.  This is always possible, since the number of edges and vertices of $G_r(\mu)$ is $\Theta(r^2)$ while the

  Consider a (for now) random permutation $\alpha_1,\ldots,\alpha_c$ of $\{1,\ldots,c\}$. For each $i\in\{1,\ldots,c\}$: Find $2(r+1)^2$ elements $\mu\in S_1$ such that
  \begin{compactenum}[(a)]
    \item
  \end{compactenum}
\end{proof}
