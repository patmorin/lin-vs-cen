...
\begin{lem}\label{only_frequent}
  ...
  for any $A\subseteq \varphi(V(G'))$, $|\varphi_{\mathcal{P}'}^{-1}(A)\cap \interior_r(G')| \ge d|A|$.
\end{lem}
...
\begin{proof}
  ...
  \begin{compactitem}
    \item For each $v:=(i,j)\in V(\interior_r(G_{k}))$ with $A\cap\varphi_\mathcal{P}(v)\neq\emptyset$, we include row $j$ in $R$ and column $i$ in $C$.
  ...
  \end{compactitem}
  At this point $|R|=|C|\le (d+2r)|A|$ and we remove all rows in $R$ and all columns in $C$ from $G$ to obtain a $k_0\times k_0$ pseudogrid $G_0$ with $k_0\ge k-d|A|$ and such that $\varphi(V(G_0))\cap A=\emptyset$.  In particular, $|\varphi(V(G_0))|\le |\varphi(V(G))|-|A|$.
...
\end{equation}
For convenience, we define $\vol{r}:=(2r+1)(6r+1)$ and the important thing to  keep in mind is that $\vol{r} \in\Theta(r^2)$.
...
\begin{lem}\label{make_disjoint}
  Let $r\ge 10$ be an integer divisible by $10$, let $G$ be a $k\times k$ pseudogrid and let $S\subseteq V(\interior_r(G))$ be such that $|\tilde{B}_{r}(v)\cap S|\le 2$ for each $v\in S$.
  Then there exists a set $X\subseteq V(G)$ such that
  \begin{compactenum}[(i)]
    \item \label{covers_s} $S\subseteq\bigcup_{x\in X}\tilde{B}_{r/10-1}(x)$;
    \item \label{two_per_box} $|\tilde{B}_{r/10-1}(x)\cap S|\le 2$ for each $x\in X$; and
    \item \label{disjoint_boxes} $\tilde{B}_{r/10}(x)\cap \tilde{B}_{r/10}(y)=\emptyset$ for each distinct $x,y\in X$.
  \end{compactenum}
\end{lem}
...
\begin{proof} 
  ...
  Let $S_1$ be the subset of $S$ containing only those vertices that do not take part in any $(r-1)$-pair.  Let $X_1:= S_1$.  Of course $S_1\subseteq \bigcup_{x\in X_1}\tilde{B}_{r/10-1}(x)$.
  
  Let $S_2$ be the subset of $S\setminus S_1$ containing those vertices that take part in a $(r/5-2)$-pair.  We now define a set $X_2$ to handle vertices in $S_2$.  For each $(r/5-2)$-pair $v,w\in S_2$, there exists a $x\in V(\interior_r(G))$ such that $v,w\in \tilde{B}_{r/10-1}(x)$, and we include $x$ in $X_2$.
  
  All that remains are the vertices in $S_3:=S\setminus (S_1\cup S_2)$ which take part in some $(r-1)$-pair but do not take part in any $(r/5-2)$ pair.  For each $(r-1)$-pair $v,w\in S_3$, there exists two vertices $x,y\in V(G)$ such that $v\in \tilde{B}_{r/10-1}(x)$, $w\in\tilde{B}_{r/10-1}(y)$ and $B_{r/10}(x)\cap B_{r/10}(y)=\emptyset$.\todo{Give details or a figure.}, and we include these in $X_3$.
  
  Let $X:=X_1\cup X_2\cup X_3$.  By construction, $X$ satisfies (\ref{covers_s}), so it only remains to show that $X$ satisfies (\ref{two_per_box}) and (\ref{disjoint_boxes}).  By construction, for any $x\in X$, $B_{r/10}(x)$ contains at least one element of $S$.   That $X$ satisfies (\ref{two_per_box}) follows easily from (\ref{covers_s}) and (\ref{disjoint_boxes}) since our construction associates each $x\in X$ with at most two elements of $S$ that are contained in $B_{r/3-1}(x)$. By (\ref{disjoint_boxes}) $B_{r/3-1}(x)$ does not intersect $B_{r/3-1}(y)$ for any $y\in X\setminus\{x\}$ so $B_{r/3-1}(x)$ does contain any elements of $S$ other than the at most two elements associated with it.  Thus, we need only argue that $X$ satisfies (\ref{disjoint_boxes}):
  \begin{compactenum}
    \item For each $x\in X_1$, $B_{r-1}(x)\cap S=\{x\}$.  Therefore, for any $v\in S\setminus\{x\}$, any box $B_{r/10}(v')$ that contains $v$ does not intersect $B_{r/10}(x)$.
    
    \item For each $x\in X_2$, $B_{r/10}(x)\cap S=\{v,w\}$ for some $(r/5-2)$-pair $v,w\in S_2$.  Furthermore, with an appropriate choice of $x$, $B_{7r/10}(x)\subseteq B_{r-1}(v)\cup B_{r-1}(w)$.  Since $(B_{r-1}(v)\cup B_{r-1}(w))\cap S=\{v,w\}$ this again implies that, for any $u\in S\setminus\{v,w\}$, any box $B_{r/10}(z)$ that contains $u$ does not intersect $B_{r/10}(x)$.
    
    \item For each $(r-1)$-pair $v,w\in S_3$, there is a corresponding pair of vertices $x,y\in X_3$. By choice, $B_{r/10}(x)$ and $B_{r/10}(y)$ do not intersect each other. $B_{r/10}(x)$ contains $v$ and $B_{r-1}(v)\cap S=\{v,w\}$.  Again, for any $u\in S\setminus\{v,w\}$, any box $B_{r/10}(z)$ that contains $u$ does not intersect $B_{r/10}(x)$.  The same argument, applied to $w$ and $y$ shows that no such box intersects $B_{r/10}(y)$. \qedhere
  \end{compactenum} 
\end{proof}


\begin{lem}\label{pick_up_everything}
  Let $r\ge 10$, let $G$ be a $k\times k$ pseudogrid with grid partition $\mathcal{P}:\{P_\mu:\mu\in\VE(G_k)\}$ and let $S\subseteq V(\interior_r(G))$ be such that $|\tilde{B}_{r}(v)\cap S|\le 2$\todo{Set constraint on $r$} for each $v\in S$. Then $G$ contains a path that contains every vertex in $S$.
\end{lem}

\begin{proof}
  Apply \cref{make_disjoint} with $r:=10\floor{r/10}$ to obtain the set $X$.  Now, consider a `snake-like' path $P_0$ that contains every row $i$ with $i\equiv 1\pmod 2r/3+1$ and, aside from those rows contains only vertices of columns $1$ and $k$.   For each $x\in X$, this path intersects exactly one row of $B_{r/3}(x)$.  For each $x\in X$ we use \cref{pick_up_two} to replace $V(P_0)\cap B_{r/3}(x)$ with a path that is contained in $B_{r/3}(x)$ and contains the (at most two) vertices of $B_{r/3}(x)\cap S$.  After doing this for each $x\in X$ we obtain a path $P$ that contains every vertex in $S$. 
\end{proof}
...
\begin{proof}
  For simplicity, assume that $G_k$ is embedded in $G_{k+2r}$ with the elements of $Q$ all contained in $\VE(\interior_{2r}(G_{k+2r})$.  In particular, for each $\mu_1\in Q$, this implies that $|B_{\rho}(\nu)| =\vol{\rho}$ for any $\rho\in\{0,\ldots,2r\}$.
...
\begin{lem}\label{doubled_colour_set}
  ...
  Let $G$ be a $k\times k$ pseudogrid with grid-partition $\mathcal{P}:=\{V(P_\mu):\mu\in\VE(G_k)\}$ and let $\varphi$ be a vertex colouring of $G$ with the property that, for each $A\subseteq\varphi(V(G))$, $|\varphi_{\mathcal{P}}^{-1}(A)\cap\interior_r(G)|\ge d|A|$.
