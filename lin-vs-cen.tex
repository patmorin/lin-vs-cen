...
\begin{proof}
  ...
  \]
  which is clearly a contradiction.  This establishes that $\vor'$ satisfies Condition~(\ref{three_in_ball}).\todo{Optimize and cleanup. Finish proving (iii)}\todo{Why not use $\beta$ (or $100\beta$) instead of $3\beta/4$?}
\end{proof}
...
\begin{proof}
  (This proof only uses the fact that $G_{a\times b})$ has no vertex of degree greater than $4$.)  Since $G$ contains a $G_{a\times b}$ minor then, by definition, $G$ contains a subgraph $H$ and a partition $\mathcal{B}:=(B_x:x\in V(G_{a\times b}))\}$ of $V(H)$ such that
  \begin{inparaenum}[(i)]
  ...
  \end{inparaenum}
  Let $(H,\mathcal{B})$ be chosen so that $H$ the minimum possible number of edges, over all choices $H$ and $\mathcal{B}$ that satisfy (i) and (ii).

  An edge of $vw$ of $H$ is a \emph{border edge} if $v\in B_x$ and $w\in B_y$ for distinct $x,y\in V(G_{a\times b})$.  For each $x\in V(G_{a\times b})$, let $\partial B_x$ denote the set of border edges incident on one vertex of $B_x$.  Since $H$ is edge-minimal, $|\partial B_x|=\deg_{G_{a\times b}}(x)\le 4$, for each $x\in V(G_{a\times b})$.  For each $x\in V(G_{a\times b})$, let $T_x\subseteq H$ be a minimal tree contained in $H[B_x]\cup \partial B_x$ that contains all the border edges in $\partial B_x$.   We define the partition $\mathcal{P}:=(P_x:x\in V(G_{a\times b}))$ as follows:  
\begin{compactenum}
  \item If $T_x$ has a node $v$ of degree $4$, then $P_x:=\{v\}$.  
  \item If $T_x$ has two nodes $v$ and $w$ of degree $3$ then $P_x$ contains all vertices on the path in $T_x$ from $v$ to $w$.
  \item If $T_x$ has exactly one node $v$ of degree $3$ then $P_{x}:=\{v\}$
  \item Otherwise $T_x$ is a path and $P_x$ contains exactly one node in $V(T_x)$.
\end{compactenum}
...
\begin{proof}
  ...
  \begin{enumerate}[{Stage} 1:]
    ...
    First observe that, for each $p\in R_1$, \cref{voronoi_mapping}(\ref{never_far}) implies that $|\vor^{-1}(p)|\le|B_{7\beta/4}(p)|\le (7\beta/2+1)^2$.  
    \todo[inline]{Fuck! We run into problems here with large colour sets.  The next line is not true, for example.   We get a $C$ in the denominator because the only degree bound we have on $R_1$ is $C(7\beta/2+1)^2$.}
    This implies that, for any $A_0\subseteq\overline{S}$,
    \[
        |N_I(A_0)| \ge \frac{a|A_0|}{(7\beta/2+1)^2} \enspace .    
    \]  
    This has two implications:
    \begin{compactenum}
      \item For each $p\in R_1$,  $\deg_{I}(x) \le C(7\beta/2+1)^2$.
      \item Since each colour $c\in A$ is frequent, $\deg_I(c)\ge \alpha/(7\beta/2+1)^2$.
    \end{compactenum}
