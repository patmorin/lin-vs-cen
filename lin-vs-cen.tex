...
Pat~Morin especially enjoyed early-morning discussions with Stefan~Langerman on the Seabourne Terrace. We are also grateful to Omer Angel for pointing out that the problem on a $k\times k$ grid (as opposed to pseudogrid) has a simple solution. (By partitioning the rows into consecutive pairs and the columns  into consecutive pairs, and then repeatedly deleting a pair of rows and columns whose intersection contains a vertex whose colour is unique, one is left with a graph that has a Hamiltonian path in which each colour apppears on at least two vertices.)
