...
\begin{lem}\label{doubled_colour_set}
  For every integer $r\ge 1$\todo{Really no lower bound on $r$?} there exists an integer $d\in O(r^4)$ such that the following is true, for every integer $k\ge 1$.
...
\begin{proof}
  ...
  We now want to use the Local Lemma (\cref{weighted_lovasz}) to show that, with probability greater than zero, $\deg_H(\alpha)\ge 2\vol{r}$ for each $\alpha\in X$.\todo{Constants are going change everywhere now...}
