...
\begin{proof} 
  ...
  Let $X:=X_1\cup X_2\cup X_3$.  By construction, $X$ satisfies (\ref{covers_s}), so it only remains to show that $X$ satisfies (\ref{two_per_box}) and (\ref{disjoint_boxes}).  That $X$ satisfies (\ref{two_per_box}) follows easily from (\ref{covers_s}) and the fact that each $x\in X$ covers at most two elements of $S$.
  
  To see that $X$ satisifies (\ref{disjoint_boxes}), observe that if $x\in X$ covers $v\in S$ then $B_{p+1}(x)\subseteq B_{2p+2}(v)$. Therefore, if $x\in X$ covers $v\in S$ and $y\in X$ covers $w\in S$ and $B_{p+1}(x)\cap B_{p+1}(y)\neq\emptyset$ then $w\in B_{4p+4}(v)$ or $v\in B_{4p+4}(w)$.  Since $r-1 \ge 4p+4$, this implies that $v=w$ or $v$ and $w$ are an $(r-1)$-pair.  In the later case, $x=y$ and there is nothing to prove. Otherwise $v,w\in S_2$ or $v,w\in S_3$.  If $v,w\in S_2$ then, again $x=y$ and there is nothing to prove.  If $v,w\in S_3$ then $x$ and $y$ are specifically chosen so that $B_{p+1}(x)$ and $B_{p+1}(y)$ are disjoint.
\end{proof}
