...
\begin{lem}
  For any $r\in\N$ there exists $\beta\in\N$ such that, for any $k\in\N$ and any $P\subseteq G^{\onesub}_k$ with $\min\{\|pq\|_\infty:p,q\in\binom{P}{2}\} \ge \beta$ there exists a function $\vor_P:V(G^{\onesub}_k)\to P$ such that
  \begin{compactenum}[(i)]
    \item for any vertex $v$ of $G^{\onesub}_k$, $|\{\vor_P(v)\}\cup\{\vor_P(w):w\in N_{G^{\onesub}_k}(v)\}|\le 2$.
    \item for any ball $B$ of radius $r$, $|\{\vor_P(v):v\in B\}|\le 3$;
  \end{compactenum}
\end{lem}
