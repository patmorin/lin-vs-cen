...
\title{\MakeUppercase{Linear Versus Centred Colouring via Pseudogrids}\thanks{This research was partly funded by NSERC.}}
\author{Prosenjit~Bose\thanks{School of Computer Science, Carleton University},\quad Vida~Dujmović\thanks{Department of Computer Science and EE, University of Ottawa},\quad Hussein~Houdrouge\footnotemark[2],\quad Mehrnoosh~Javarsineh\footnotemark[2],\quad and Pat~Morin\thanks{School of Computer Science, Carleton University, \email{morin@scs.carleton.ca}}}
...
\keywords{treedepth; graph colouring; centered colouring; linear colouring}
...
\begin{conj}[\cite{kun.obrien.ea:polynomial}]\label{crazy_conjecture}
...
\begin{figure}
  \begin{center}
    \includegraphics[page=2]{figs/pseudogrid}
  \end{center}
...
\begin{figure}
  \begin{center}
    \begin{tabular}{ccc}
      \includegraphics[page=2]{figs/outline} &
      \includegraphics[page=3]{figs/outline} &
      \includegraphics[page=4]{figs/outline} \\
      $S_1$ & $S_1\cup S_2$ & $P$
    \end{tabular}
  \end{center}
...
\begin{figure}
  \begin{center}
    \begin{tabular}{ccc}
      \includegraphics[page=1]{figs/bad-example} &
      \includegraphics[page=2]{figs/bad-example} &
      \includegraphics[page=3]{figs/bad-example}
    \end{tabular}
  \end{center}
...
% \todo{Do we really need $\interior_r(G)$ or can we use $\interior_2(G)$?  We could change it, but let's not.}
...
\begin{proof}
  ...
  \begin{figure}
    \begin{center}
      \begin{tabular}{ccc}
        \includegraphics[page=1,width=.3\textwidth]{figs/make_disjoint} &
        \includegraphics[page=2,width=.3\textwidth]{figs/make_disjoint} &
        \includegraphics[page=3,width=.3\textwidth]{figs/make_disjoint} \\
        $X_1$ & $X_2$ & $X_3$
      \end{tabular}
    \end{center}
...
\begin{proof}
  Fix a real number $c\ge 1$ such that $d=cr^4\log r$ is an integer.
  ...
  For each $\alpha\in X$, let $\Phi_\alpha:=\phi^{-1}(\alpha)\cap \left(\bigcup_{\mu\in Y}\VE(G_{2r+1}(\mu))\right)$.  In words, $\Phi_\alpha$ consists of all objects of colour $\alpha$ that were eliminated from consideration because they are too close to an element of $Y$. We claim that, for each $\alpha\in X$, $|\Phi_\alpha|\ge d-\vol{2r+1}$.  Otherwise $|\phi^{-1}(\alpha)\setminus\Phi_\alpha| > \vol{2r+1}$. Then, when the colour $\alpha$ was considered during the first round, we could have taken $\mu_1$ to be any object in $\phi^{-1}(\alpha)\setminus\Phi_\alpha$ and taken $\mu_2$ to be any object in $\phi^{-1}(\alpha)\setminus\Phi_\alpha\setminus B_{2r+1}(\mu_1)$. (Recall that $\vol{2r+1}\ge |B_{2r+1}(\mu_1)|$, by definition.)  Thus, the first round would have been an $\alpha$-success, contradicting the fact that $\alpha\in X$.
  ...
  \begin{align}
    ...
    & > \tfrac{1}{\tau}\left(1-\tfrac{1}{\tau}\right)^{|\Gamma_\alpha|}
      & \text{(since $1-1/\tau<1$)}\notag \\
    % & = \exp(-\ln\tau)\cdot \left(1-\tfrac{1}{\tau}\right)^{|\Gamma_\alpha|} \notag \\
    % & = \exp(-\ln\tau)\cdot \left(1-\tfrac{1}{\tau}\right)^{|\Gamma_\alpha|} \notag \\
  ...
  provided that
  \begin{equation}
    |\Phi_\alpha| \ge {32\vol{7r}}\big(\ln({32\vol{7r}}+1) + {2\vol{2r+1}}\ln(2|\Phi_\alpha|)\big) \enspace . \label{critical}
  \end{equation}
  The right hand side of this last equation is at most $c'(r^2\log r+r^4\log|\Phi_\alpha|) \le c''r^4\log|\Phi_\alpha|$ for some absolute constants $c'$ and $c''$.  Thus, it suffices to choose $c$ so that
  \[
    \frac{|\Phi_\alpha|}{\log|\Phi_\alpha|}
    \ge \frac{(d-\vol{r})}{\log(d-\vol{r})}
    \ge \frac{(d-\vol{r})}{\log d}
    = \frac{cr^4\log r-\vol{r}}{\log(cr^4\log r)}
    \ge \frac{cr^4\log r-\vol{r}}{\log(cr^5)}
    \ge \frac{cr^4\log r}{2\log(cr^5)}
    \ge c''r^4 \enspace .
  \]
  Rewriting the last inequality gives $c-2\log_r c\ge 10c''$.  Since $r\ge 10$, $c-2\log_r c\ge c/2$, for all $c\ge 1$.  So \eqref{critical} holds for all $c\ge 20c''$.
...
\begin{figure}
  ...
  \begin{tabular}{cccc}
    ...
    % \includegraphics[page=3]{figs/omer} &
...
\begin{proof}[Proof of \cref{pick_up_two}]
  ...
  \begin{figure}[htbp]
    \begin{center}
      \begin{tabular}{cc}
        \includegraphics[page=1]{figs/generic_path} &
        \includegraphics[page=2]{figs/generic_path}
      \end{tabular}
    \end{center}
