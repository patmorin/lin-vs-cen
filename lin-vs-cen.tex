...
A recent result of \citet[Theorem~1.4]{czerwinski.nadara.ea:improved} shows that if $b$ is the maximum treedepth of any tree contained in $G$ then $\td(G)\le b\cdot\tw(G)$ (and the proof is easy).  If we take $\tw(G)=k^{38}$ and $b=k$ then, for any graph $G$ with $\td(G)\ge k^{39}$, one of the following cases applies:
\begin{enumerate}
...
\todo[inline]{I can't yet prove the full result, which is why the next lemma has some weird condition involving a newly introduced parameter $r$.}
