...
In \cref{only_frequent_sec} we explain how to remove rows and columns of $G$ to obtain a sub-pseudogrid in which every colour appears frequently.  In \cref{path_finding} we explain how, given a well-separated set $S$ of vertices in $G$, to find a path that contains every vertex in $S$.
% In \cref{packing_lemma_sec} we introduce a fairly standard-looking packing lemma that is needed in several places later.
In \cref{well_separated} we explain how find a well-separated set $S$ that contains two vertices of each colour. Finally \cref{wrapping_up} shows how to assemble these various pieces to prove \cref{pseudogrid_lower_bound}.
...
Later, we will make use of the following observation about sets of boxes, none of which contains the center of any other.  (Intuitively, each of the four quadrants surrounding $\nu$ can contain at most one element $\mu\in Q$ with $\nu\in \VE(G_r(\mu))$.)
\begin{obs}\label{packing_lemma}
  Let $r,k\ge 1$ be integers and let $Q\subseteq\VE(G_k)$ be such that $\VE(G_r(\mu))\cap Q=\{\mu\}$ for each $\mu\in Q$.  Then, for each $\nu\in\VE(G_k)$, $|\{\mu\in Q:\nu\in \VE(G_r(\mu))\}|\le 4$.
\end{obs}

We extend the definition of boxes to vertices of a $k\times k$ pseudogrid $G$ with grid-partition $\mathcal{P}:=\{V(P_\mu):\mu\in\VE(G_k)\}$ as follows. For any $\mu\in\VE(G_k)$, define
\[
...
\begin{proof}
  ...
  \begin{align}
    ...
    & = \sum_{x=0}^{2\vol{2r+2}} \binom{t}{x}\cdot (1/4)^x \cdot (3/4)^{t-x} \notag \\
    & \le \sum_{x=0}^{2\vol{2r+2}} \binom{t}{x}\cdot (3/4)^{t} \notag  \\
    & \le (2t)^{2\vol{2r+2}}\cdot (3/4)^{t} \notag \\
    % & \le \exp(6(2r+1)^2\log(2t)+(t-6(2r+1)^2)\log(1-1/25)) \\
    & < \exp\left({2\vol{2r+2}}\ln(2t)- t/4\right)
      & \text{(since $\ln(3/4) < -1/4$)}\notag \\
    & \le \exp\left({2\vol{2r+2}}\ln(2|\Phi_\alpha|) -|\Phi_\alpha|/4\vol{5r}\right)
      & \text{(since $|\Phi_\alpha|/\vol{5r}\le t\le |\Phi_\alpha|$)}\label{probability}
  ...
  We are now ready for an application of \cref{weighted_lovasz}.  For each $\alpha\in X$, let Let $E_\alpha$ denote the event ``$\deg_H(\alpha)\le 2\vol{2r+2}$'' and let $\mathcal{E}:=\{E_\alpha:\alpha\in X\}$. For each $\alpha\in X$, define
  \[
     \Gamma_\alpha := \left\{E_\beta \in \mathcal{E}: \Phi_\beta\cap \bigcup_{\nu\in \Phi_\alpha} \VE(G_{5r}(\nu))\neq\emptyset\right\}
  \]
  and observe that $|\Gamma_\alpha|\le \vol{5r}\cdot|\Phi_\alpha|$.
  The event $E_\alpha$ is mutually independent of $\mathcal{E}\setminus\Gamma_\alpha$ since, for any $E_\beta\in\mathcal{E}\setminus\Gamma_\alpha$ the sets $Y_\alpha:=\{\mu\in Y:B_{2r+2}(\mu)\cap \Phi_\alpha\neq\emptyset\}$ and $Y_\beta:=\{\mu\in Y:B_{2r+2}(\mu)\cap \Phi_\beta\neq\emptyset\}$ are disjoint and $\deg_H(\alpha)$ and $\deg_H(\beta)$ are each determined by the relative orders of $Y_\alpha$ and $Y_\beta$ in $\pi$, respectively.

  For each $\alpha\in X$, let $w(E_\alpha):=1/\tau$, for some $\tau>1$ to be determined shortly.  Then
  \begin{align}
    ...
    & \ge \exp\left(-\log\tau - \vol{5r}\cdot|\Phi_\alpha|/(\tau-1)\right) \enspace . \label{weight_product}
  \end{align}
  ...
  Taking $\tau = {8\vol{5r}}+1$, and using \cref{weight_product,probability}, we find that
  \[
     \Pr\left(E_\alpha\right) \le w(E_\alpha)\prod_{\beta\in\Gamma_\alpha}(1-w(\Gamma_\alpha))
  \]
  provided that
  \[   |\Phi_\alpha| \ge {8\vol{5r}}\big(\log({8\vol{5r}}+1) + {2\vol{2r+2}}\log(2|\Phi_\alpha|)\big) \enspace .
  \]
  The right hand side of this last equation is of the form $O(r^2\log r+r^4\log|\Phi_\alpha|)$.  Since $|\Phi_\alpha|\ge d-\vol{2r+2}$, this can therefore be satisfied for some $d\in O(r^4\log r)$.
  ...
  Therefore, by \cref{weighted_lovasz}, there exists a permutation $\pi$ of $Y$ that produces a bipartite graph $H$ such that $\deg_H(\alpha)\ge {2\vol{2r+2}}$ for each $\alpha\in X$.  On the other hand, $\deg_{H}(y)\le |\VE(G_{2r+2}(y))|-1 < {\vol{2r+2}}$ for each $y\in Y$.  Therefore, for any $A\subseteq X$, $|N_H(A)|\ge {2\vol{2r+2}}|A|/{\vol{2r+2}} = 2|A|$.  Therefore, by \cref{d_hall}, there is a subgraph $M$ of $H$ in which $\deg_M(\alpha)=2$ for each $\alpha\in X$ and $\deg_M(\mu)\le 1$ for each $\mu\in Y$. Each edge $\alpha\mu$ of $M$ corresponds to some $\nu\in X_\alpha\cap\VE(G_r(\mu))$, which we place in $Q_2$.
